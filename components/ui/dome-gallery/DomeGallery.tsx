...
function buildItems(pool: MediaItem[], seg: number): ItemDef[] {
  ...
  // 9:16 aspect ratio (approx 2 : 3.5)
  const evenYs = [-7, -3.5, 0, 3.5, 7];
  const oddYs = [-5.25, -1.75, 1.75, 5.25];

  const coords = xCols.flatMap((x, c) => {
    const ys = c % 2 === 0 ? evenYs : oddYs;
    return ys.map(y => ({ x, y, sizeX: 2, sizeY: 3.5 }));
  });

  const totalSlots = coords.length;
  if (pool.length === 0) {
    return coords.map(c => ({ ...c, type: 'image' as const, src: '', alt: '' }));
  }
  
  // Fill slots by cycling through the pool
  const usedItems = Array.from({ length: totalSlots }, (_, i) => pool[i % pool.length]);

  // Shuffle adjacent duplicates to avoid identical neighbors
  for (let i = 1; i < usedItems.length; i++) {
    if (usedItems[i].src === usedItems[i - 1].src) {
      for (let j = i + 1; j < usedItems.length; j++) {
        if (usedItems[j].src !== usedItems[i].src) {
...
export default function DomeGallery({
  ...
  const openItemFromElement = useCallback((el: HTMLElement) => {
    ...
    const type = parent.dataset.type || 'image';
  ...
  useEffect(() => {
    ...
    const close = () => {
      ...
      // Clone content for closing animation
      const originalContent = overlay.querySelector('img') || overlay.querySelector('video');
      if (originalContent) {
        ...
        if (clone instanceof HTMLVideoElement) {
            // Ensure video keeps playing or shows poster
            clone.muted = true;
